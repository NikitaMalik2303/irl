    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        if self.sample_strategy == 'random' \
            ...
            for i in range(self.cem_iterations):
                ...
                if i==0:
                    candidate_action_sequence= np.random.uniform(self.low, self.high, (num_sequences, horizon, self.ac_dim))
                else:
                    candidate_action_sequence= np.random.normal(mean, std, size=(num_sequences, horizon, self.ac_dim))
                
                elite_idx= np.argsort(self.evaluate_candidate_sequences(candidate_action_sequence, obs))[-self.cem_num_elites:]
                elites= candidate_action_sequence[elite_idx]
